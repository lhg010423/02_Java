	...
	public void displayMenu() {
		...
		do {
			...
			try {
				...
				menuNum = -1;
				// 첫 반복에서 예외 발생 시 종료시키지 않도록 menuNum 값을 임의의 값으로 세팅
			}
...
//		toy.add( new Toy (name, age, price, color, madeymd, madeUseSum));
	...
	public void displayToysByAge() {
		...
		for(Toy toy : toySet) {
			...
			// age == 5 이면 key가 5인 리스트가 없으면 리스트 생성
...
//			searchAge = toy.get(i).getAge();
//			System.out.printf("[연령:%d세]\n", age);
//			
//			for(int a = 0; a < toy.size(); a++) {
//				if(toy.get(i).getAge() == searchAge) {
	...
	public void removeMaterialByName() throws Exception{
		...
		for(Entry<Integer, String> entry : materialMap.entrySet()) {
			...
			if(entry.getValue().equals(materialName)) {
				materialMap.remove(entry.getKey()); // Key를 전달하면 전부 삭제
