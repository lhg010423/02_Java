...
public class MemberService {
	...
	public String signUp() {
		...
		int index = emptyIndex(); // memberArr 배열에서 비어있는 인덱스 반환받음 (없다면 -1반환)
...
//		for(int i = 0; i < memberArr.length; i++) {
//			
//			// memberArr[i] 에 값이 없을때 넘어감,
//			// 이거안쓰면 밑에 if문에 값이 없는데 출력해야해서 오류남
//			if(memberArr[i] == null) {
//				continue;
//			}
//			
//			if(memberArr[i].getMemberName().equals(inputName)) {
//				System.out.printf("아이디 : %s, 비밀번호 : %s, 이름 : %s,\n나이 : %d, 지역 : %s\n",
//						memberArr[i].getMemberId(), memberArr[i].getMemberPw(), memberArr[i].getMemberName(),
//						memberArr[i].getMemberAge(), memberArr[i].getRegion());
//				flag = false;
//			}
//			
//		}
