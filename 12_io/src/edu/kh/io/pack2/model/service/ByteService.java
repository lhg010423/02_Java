...
public class ByteService {
	...
	// -> IOException 예외 처리 구문으 작성하는 것이 필수!
	/**************************************************/
	...
	public void fileByteOutput() {
		...
		FileOutputStream fos = null; // 보통 바로 new 를 잘 안씀
		// new 쓰면 file not fount 예외 생김 그래서 아래처럼 try안에 씀
		
		try {
			// new FileOutputStream("경로");
			// -> 경로에 작성된 파일과 연결된 파일 출력 스트림 객체 생성
			// -> 출력 스트림에 연결된 파일이 존재하지 않으면 자동 생성
			// (단, 폴더는 생성되지 않음, 파일만 가능)
			...
			// -> 2byte 범주에 문자(영어, 숫자, 기본 특수문자 제외)는
			//    1byte 기반 스트림 통과 시
			...
			// 1ms(밀리)     == 1/1000초
			// 1us(마이크로) == 1/1000ms
			// 1ns(나노)     == 1/1000us (1/10억 초) 외울필요 없음
	...
	public void bufferedFileByteOutput() {
		...
		BufferedOutputStream bos = null; // 보조 스트림 선언, 보조해주는거라 단독으로 못씀
	...
	public void fileByteInput() {
		...
		// file not found 가 발생하니 try를 쓴다
		try {
			
			// 노래가사가 한글이있으면 깨지고 영어면 안깨진다
			fis = new FileInputStream("\\io_test\\20240222\\노래가사.txt");
			//        FileInputStream 이 자동으로 파일을 생성해주지는 않음
			//        그래서 미리 직접 ㄴ파일을 만들어놓고 함
			
			// 방법 1. 파일 내부 내용을 1byte씩 끊어서 가져오기
			// -> 2byte 범주의 글자들이 깨지는 문제가 발생
			
			// 자바에서 byte는 정수형
			// -> 다루기 힘들기때문에 정수 기본형이 int로 변환해서 사용
	...
	/**
	 * BufferedInputStream 보조 스트림을 니용한 성능 향상
	 */
	...
	public void fileCopy() {
		...
		// 스트림 참조 변수 선언 5개씀
		// 읽어올때 기본, 보조 스트림2개 출력할때도 2개, 마지막에 BufferedReader
		...
		FileOutputStream fos = null; // 파일 입력
