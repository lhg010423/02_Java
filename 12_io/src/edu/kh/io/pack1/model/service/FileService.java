...
public class FileService {
	...
	public void method1() {
		...
		if ( !directory.exists() ) { // 폴더가 존재하지 않으면
			...
			System.out.println(directory.getPath()); // \io_test\20240222, 전체경로
	...
	public void method3() {
		...
		for( File f : files) {
			...
			// 마지막으로 수정한 날짜, 반환형이 long 타입이라 그냥 못씀 변수에 담아야함
			long lastModified = f.lastModified();
			// System.out.println(lastModified); // 1709388653187
			
			// SimpleDateFormat : 간단히 날짜 형식으 지정할 수 있는 객체
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd a h:mm");
			//                    2024-02-22 오후 2:14                a : 오전 오후 구별
