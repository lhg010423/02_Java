...
public class StudentService {
	...
	// * 인터페이스 객체 생성 못함 X, 부모 참조변수 역할 가능O
	
	// java.utill.ArrayList : 배열 형태 List (가장 대표적인 List 자식 클래스)
	
	// ArrayList() 기본생성자 : 기본 크리 10짜리 리스트 생성
	...
	// 미완성 설계도라서 new List() 불가능
	private List<Student> studentList = new ArrayList<Student>(); // 검색(조회)에 효육적
//	private List<Student> studentList = new LinkedList<Student>(); // 추가, 수정, 삭제에 효율적
	...
	public StudentService() { // 기본생성자
		...
		studentList.add( new Student("길동일", 24, "인천시 미추홀구", 'M', 100) );
		studentList.add( new Student("길동이", 23, "서울시 강남", 'f', 80) );
		studentList.add( new Student("길동삼", 22, "서울시 서대문구", 'M', 70) );
		studentList.add( new Student("길동사", 21, "경기도 안산시", 'F', 60) );
	...
	public void selectAll() {
		...
		if(studentList.isEmpty()) { // 컬렉션이 비어있다면 true 반환
			...
			return; // 현제 메소드를 종료하고 호출한 곳으로 돌아간다
					// 단, 반환값은 없다(void)
		}
		...
		// 작성법
		// for( 컬렉션 또는 베열에서 꺼낸 한개의 요소를 저장할 변수 : 컬렉션명 또는 배열명 ) {}
	...
	/**
	 * 학생 정보 수정 메소드
	 * - 학생정보가 studentList에 있는지 겁사, 없다면 "입력된 학생 정보가 없습니다" 문자열 반환
	...
	public String updateStudent() throws InputMismatchException{
		...
		// 1) 학생정보가 studentList에 있는지 겁사, 없다면 "입력된 학생 정보가 없습니다" 문자열 반환
		...
		// 4) 만약 문자열을 입력한 경우 -> 예외처리 throws
			...
			// index 번째에 있는 기존 학생 정보가 반환되고, 그 객체를 temp에 저장한다
...
//		System.out.println("검색 결과가 없습니다");
	 ...
	 * 문자열을 입력받아 studentList에서 꺼내온 Student 객체의 name값에 포함되는 문자열인지 검사
	...
	public void searchName2() {
		...
		System.out.print("이름에 포함된 한글자 입력 : ");
		String input = sc.next();
		boolean flag = true;
...
//		for(int i = 0; i < studentList.size(); i++) {
//			String name = studentList.get(i).getName();
//			if(boolean String.contain(studentList.get(i)))
//		}
	...
	public void sortByAge() {
		...
		// Student class 에서 클래스명 옆에 implements Comparable<Student> 안하면 오류남
	...
	public void sortByName() {
		...
		// Student::getName : 메서드 레퍼런스(Methode Reference)
